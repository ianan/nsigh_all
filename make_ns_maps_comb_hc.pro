pro make_ns_maps_comb_hc,obs_id=obs_id,maindir=maindir,nsdir=nsdir,fpm=fpm
  ...
  if (obs_id eq 29) then begin
    ; Just separate out the mosaics
    gd_ids=intarr(n_elements(evtaf))+1
    iidds=strmid(evtaf,strpos(evtaf[0],'nu206')+4,9)
    gd_ids[where(iidds ge 625005001 and iidds le 625023001)]=2
  ...
  if (obs_id eq 33) then begin
    ; OK to combine all
    gd_ids=intarr(n_elements(evtaf))+1
  endif
  
  if (obs_id eq 34) then begin
    ; OK to combine all
    gd_ids=intarr(n_elements(evtaf))+1
  endif
  
  if (obs_id eq 35) then begin
    ; Split per obs target id
    gd_ids=intarr(n_elements(evtaf))+1
  endif
  
  if (obs_id eq 36) then begin
    ; Split per obs target id
    gd_ids=intarr(n_elements(evtaf))+1
    iidds=strmid(evtaf,strpos(evtaf[0],'nu2101')+3,9)
    gd_ids[where(iidds eq 101200400)]=2
    gd_ids[where(iidds ge 101200500 and iidds le 101200600)]=3
    gd_ids[where(iidds ge 101200700 and iidds le 101200800)]=4
  endif
  
  if (obs_id eq 37) then begin
    ; Split per obs target id
    gd_ids=intarr(n_elements(evtaf))+1
  endif
